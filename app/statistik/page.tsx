...
import { buildMissionPlacementTimeline } from "@/lib/statistics/mission-placement-timeline";
...
function MissionsArea({ statistics, timeline }: { statistics: ReturnType<typeof calculateMissionStatistics>; timeline: ReturnType<typeof buildMissionPlacementTimeline> }) {
...
export default async function StatisticsPage({ searchParams }: { searchParams: Promise<{ bereich?: string }> }) {
  ...
  return <main className={styles.page}><header className={styles.hero}><span>Globale Auswertung</span><h1>Statistiken</h1><p>Rekorde, Serien und Kennzahlen aus allen bestätigten Partien der Spielgruppe.</p></header><nav className={styles.tabs} aria-label="Statistikbereiche">{[["spieler", "Spieler"], ["spiel", "Spiel"], ["missionen", "Missionen"]].map(([key, label]) => <Link key={key} href={`/statistik?bereich=${key}`} aria-current={area === key ? "page" : undefined}>{label}</Link>)}</nav>
    {area === "spieler" && <PlayersArea statistics={calculatePlayerStatistics(publicPlayers, games)} />}{area === "spiel" && <GamesArea statistics={calculateGameStatistics(games)} />}{area === "missionen" && <MissionsArea statistics={calculateMissionStatistics(games, missions)} timeline={buildMissionPlacementTimeline(games, missions)} />}
  </main>;
}
