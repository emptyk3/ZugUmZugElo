...
function MissionsArea({ statistics }: { statistics: ReturnType<typeof calculateMissionStatistics> }) {
  return <section className={`${styles.tableCard} ${styles.missionTableCard}`}><h2>Missionsstatistiken</h2><div className={`${styles.tableWrap} ${styles.missionTableWrap}`}><table><colgroup><col className={styles.missionNameColumn} /><col className={styles.missionDrawnColumn} /><col span={10} /></colgroup><thead><tr><th>Mission</th><th>Gezogen</th><th>% Gezogen</th><th>Behalten</th><th>% Behalten</th><th>Siege</th><th>Sieg-%</th><th>Ø Platz</th><th>Ø Punkte</th><th>Ø Punkte (Sieg)</th><th>Ø Elo ±</th><th>Max. Punkte</th></tr></thead><tbody>{statistics.rows.map((row, index) => <tr key={row.id}><th>{index < 3 ? missionMedals[(index + 1) as 1 | 2 | 3] : `${index + 1}.`} {row.name}</th>
