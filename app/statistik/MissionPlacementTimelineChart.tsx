...
import type { MissionPlacementTimeline } from "@/lib/statistics/mission-placement-timeline";
...
export default function MissionPlacementTimelineChart({ series, entries, maximumPlacement }: Props) {
  ...
  const placementsByMission = useMemo(() => new Map(series.map((mission) => [mission.id, data.flatMap((game) => game.missionValues[mission.id].placements.map((result) => ({ ...result, gameId: game.gameId, playedAt: game.playedAt, timestamp: game.timestamp, visualPosition: game.visualPosition })))])), [data, series]);
  ...
  return <article className={`${styles.timelineCard} ${styles.missionTimelineCard}`}>
    ...
    {!data.length ? <div className={styles.timelineEmpty}>Noch keine Missionsdaten vorhanden.</div> : <div className={styles.missionTimelineChartScroll}><div className={styles.missionTimelineChart} role="img" aria-label="Platzierungsentwicklung der ausgewählten Missionen">
      <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 12, right: 12, bottom: 8, left: 4 }}>
        ...
        <Tooltip shared content={({ active, payload }) => { const payloadPoint = payload?.[0]?.payload as { gameId?: string } | undefined; const game = payloadPoint?.gameId ? gamesById.get(payloadPoint.gameId) : undefined; const shownMissions = game ? series.filter((mission) => (visible.has(missionLineKey(mission.id, "result")) || visible.has(missionLineKey(mission.id, "average"))) && game.missionValues[mission.id].placements.length > 0) : []; return active && game && shownMissions.length ? <div className={`${styles.timelineTooltip} ${styles.missionTimelineTooltip}`}><strong>{fullDate(game.timestamp)}</strong>{shownMissions.map((mission) => { const value = game.missionValues[mission.id]; return <div key={mission.id}><b>{mission.name}</b>{value.placements.length === 1 ? <span>Platz {value.placements[0].placement}</span> : value.placements.map((result) => <span key={`${result.participantId}-${result.placement}`}>{result.playerAlias} – Platz {result.placement}</span>)}<span>Laufender Ø Platz: {average(value.cumulativeAveragePlacement!)}</span></div>; })}<Link href={`/partien/${game.gameId}`}>Partie öffnen</Link></div> : null; }} />
        {series.flatMap((mission, index) => { const color = palette[index % palette.length]; return [
          visible.has(missionLineKey(mission.id, "result")) && <Scatter key={`${mission.id}-result`} data={placementsByMission.get(mission.id)} dataKey="placement" name={`${mission.id}:result`} fill={color.light} line={{ stroke: color.light, strokeWidth: 1.5, strokeOpacity: .76 }} shape="circle" isAnimationActive={false} />,
