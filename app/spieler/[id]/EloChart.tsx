...
const rating = (value: number) => new Intl.NumberFormat("de-AT", { maximumFractionDigits: 1 }).format(value);
...
export default function EloChart({ points }: { points: EloPoint[] }) {
  ...
  return <div className={styles.chart} aria-label="Elo-Verlauf">
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 14, right: 10, bottom: 8, left: -18 }}>
        ...
        <Tooltip content={({ active, payload }) => { const point = payload?.[0]?.payload as typeof data[number] | undefined; return active && point ? <div className={styles.tooltip}>
          ...
          <span>Elo vorher: {rating(point.ratingBefore)}</span><span>Änderung: {point.ratingChange > 0 ? "+" : ""}{rating(point.ratingChange)}</span><span>Elo danach: {rating(point.ratingAfter)}</span>
